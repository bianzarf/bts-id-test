...
const searchSchema = Joi.object().keys({
    todo_detail_name: Joi.string().max(200).optional(),
}).unknown(true)

const insertSchema = Joi.object().keys({
    todo_id : Joi.string().max(200).required(),
    todo_detail_name : Joi.string().max(200).required(),
}).unknown(true)
const updateSchema = Joi.object().keys({
    todo_detail_id : Joi.string().max(50).required(),
    todo_detail_name : Joi.string().max(200).required(),
}).unknown(true)
